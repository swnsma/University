...
class AllGroupRemove_Test extends PHPUnit_Extensions_Selenium2TestCase
{
    ...
    public function testScenario()
    {
        $this->addGroups();
        $this->openGroupList();
        $deleted = $this->deleteRandomGroup();
        $this->submitDeletion();
        $this->openGroupList();
        $this->assertFalse($this->verify($deleted));
    ...
    public function deleteRandomGroup()
    {
        $toDelete = $this->list[rand(0, count($this->list) - 1)];
        $element = $this->element($this->using('css selector')->value('input[title="Select (' . $toDelete . ')"' . "]"));
        $element->click();
        return $toDelete;
    }
